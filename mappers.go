...
func (d *mappers) Merge(other MapperGetter) error {
	...
	if !ok {
		return fmt.Errorf("can not merge %T, must be a Mappers generated by the sesame", other)
	}
	...
	ms.UnsafeFuncIndex().Range(func(k, v interface{}) bool {
		lv := v.([]mfunc)
...
func GetMapperFunc[T any, U any](mappers MapperGetter,
	...
	if !ok {
		return nil, fmt.Errorf("function is not a mapper function")
	}
	return v, nil
}
