...
func (m *MapperFuncField) Signature(mctx *MappingContext) string {
	return fmt.Sprintf("func(%s.Context, %s, %s) error",
		...
		GetStructPointerTypeSource(m.Source, mctx),
		GetStructPointerTypeSource(m.Dest, mctx))
}
...
func (c *MappingContext) AddMapperFuncField(sourceType types.Type, destType types.Type, fid FuncID) {
	...
	mapperFuncName := funcObjectName(sourceType, destType, fid)
	for _, m := range c.mapperFuncFields {
		if m.internalName == mapperFuncName {
			return
		}
	}
...
func (c *MappingContext) AddConverterFuncField(sourceType types.Type, destType types.Type, fid FuncID) {
	...
	converterFuncName := funcObjectName(sourceType, destType, fid)
	for _, m := range c.converterFuncFields {
		if m.internalName == converterFuncName {
			return
		}
	}
	...
	c.converterFuncFields = append(c.converterFuncFields, &ConverterFuncField{
		FieldName:    fieldName,
		internalName: converterFuncName,
		Source:       sourceType,
		Dest:         destType,
		ObjectID:     fid.ObjectID(),
	})
...
func funcObjectName(sourceType, destType types.Type, fid FuncID) string {
	if string(fid) != "" {
		return string(fid)
	}
	return fmt.Sprintf("$global:%s:%s", GetQualifiedTypeName(sourceType), GetQualifiedTypeName(destType))
}
